/* Nes/Snes/Genesis/SMS/Atari to USB (9 buttons modification)
 * Copyright (C) 2015 Alfonso Jiménez
 *
 ...
 *
 * The author may be contacted at yo@alfonsojimenez.net
...
static void readController(unsigned char bits[3])
{
	bits[0] = PINC;
	bits[1] = PINB;
	bits[2] = PIND & (1<<PD7);
...
static void twelveInit(void)
{
  ...
  // Marking D7 as input, the rest stays the same
  DDRD &= ~(1 << PD7);
	PORTD |= (1 << PD7);
